...
pub struct DataBlock<'a, T> {
    header: Header,
    data_ptr: *mut u8,
    _object: &'a PhantomData<T>,
}

// Unit is byte.
pub const HEADER_LEN: usize = 4 * 4;

unsafe impl<T> Send for DataBlock<'_, T> {}
unsafe impl<T> Sync for DataBlock<'_, T> {}
...
impl<'a, T> DataBlock<'a, T> {
    ...
    /// [`DataBlock`].
    pub(crate) unsafe fn new(start_ptr: *mut u8, len: u32) -> Result<Self> {
    ...
    pub(crate) unsafe fn from_raw(start_ptr: *mut u8) -> Self {
        let header = Header::fow_raw(start_ptr);
...
#[cfg(test)]
mod tests {
    use crate::ringbuf::data_block::{DataBlock, HEADER_LEN};
    ...
    fn test_data_block() {
        ...
        let data_block: DataBlock<()> =
            unsafe { DataBlock::new(data_ptr, 1024) }.unwrap();
        ...
        let data_block: DataBlock<()> =
            unsafe { DataBlock::from_raw(data_ptr) };
